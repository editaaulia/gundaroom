  ...
  <body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light">
        ...
        <?php 
            ...
            if($proses=="hapus"){
                $topik_id=mysqli_real_escape_string($kon,$_GET['topik_id']);
            
                $hapus=mysqli_query($kon,"DELETE FROM topik WHERE topik_id='$topik_id'");
                if($hapus){
    ...
    <div class="container">
        ...
        <table class="table table-bordered tabel-user">
            ...
            <tbody>
                <?php
                $i=1;
                    $tampil=mysqli_query($kon,"SELECT * FROM topik");
                    while($cetak=mysqli_fetch_array($tampil)){
                ?>
                <tr>
                    <td><?php echo $i?></td>
                    <td><?php echo $cetak['title'];?></td>
                    <td><?php echo $cetak['deskripsi'];?></td>
                    <td><a href="data-forum.php?topik_id=<?php echo $cetak['topik_id']; ?>&&proses=hapus" class="btn btn-danger">Delete Topik</a></td>
                </tr>
                <?php $i=$i+1; }?>
            </tbody>
        </table>
